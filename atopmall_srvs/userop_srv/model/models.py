...
db = ReconnectMysqlDatebase(database="atopmall_userop_srv",host="192.168.1.106",port=3306,user="root",password="123456")
...
class UserFav(BaseModel):
    ...
    class Meta:
        primary_key = CompositeKey('user', 'goods') #这一行表示用户收藏的主键是用户id和商品id的组合，只有当用户id和商品id都相同时，才认为是同一个收藏记录
