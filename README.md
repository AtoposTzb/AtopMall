...
│       ├── config/                   # 配置结构体定义
│       ├── forms/                    # 请求表单验证
│       ├── global/                   # 全局变量（配置、Redis、翻译器）
│       ├── initialize/               # 初始化（配置加载、路由、日志、Redis）
...
| 通过id查询用户    | GetUserById     | 验证用户是否存在                 |
| 通过email查询用户 | GetUserByEmail  | 验证用户是否存在                 |
| 根据mobile查询    | GetUserByMobile | 验证用户是否存在                 |
| 创建用户          | CreateUser      | 注册新用户，密码使用 PBKDF2 加密 |
| 更新用户信息      | UpdateUser      | 检查用户是否存在                 |
...
| 用户列表       | GET /user/list   | 获取用户列表（需 JWT 认证）                |
