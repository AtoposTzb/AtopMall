...
func SrcClientInitBL() {
	...
	global.OrderSrvCli = &global.OrderRpcClient{
		Order:        proto.NewOrderClient(orderConn),
		ShoppingCart: proto.NewShoppingCartClient(orderConn),
	}
	zap.S().Infow("[SrcClientInitBL] 连接【订单服务】成功", "host", consulInfo.Host, "port", consulInfo.Port)
	...
	zap.S().Infow("[SrcClientInitBL] 连接【库存服务】成功", "host", consulInfo.Host, "port", consulInfo.Port)
}
